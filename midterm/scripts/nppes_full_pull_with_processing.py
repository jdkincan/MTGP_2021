...
def json_data_to_df(json_data: dict) -> pd.DataFrame:
    ...
    main_results_df = pd.json_normalize(json_data['results'])[['basic.name', 'basic.status', 'number', 'created_epoch', 'last_updated_epoch']]
    ...
    if skip < max_results:
        ...
        if  result_count > 0:
            ...
            if result_count == limit:
                ...
                time.sleep(5)
                return get_results(taxonomy, result_set, skip, limit, max_results)
...
full_result_set.to_csv ('../data/nppes.csv', index = None, header=True)
